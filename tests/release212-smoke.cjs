...
for(const term of ['v2.13.0','releaseLocked','Object.defineProperty'])assert.ok(version.includes(term),term);
for(const asset of ['workout-experience.js?v=20260916-release212','release-212.css?v=20260916-release212']){assert.ok(html.includes(asset),asset);assert.ok(sw.includes(asset),asset)}
assert.ok(html.includes('release-version.js?v=20260917-release213'));
assert.ok(sw.includes('release-version.js?v=20260917-release213'));
assert.equal(pkg.version,'2.13.0');
console.log('PASS v2.12 workout experience features remain wired inside the v2.13 shell');