(()=>{
  ...
  function ordinal(n){if(n===1)return'first';if(n===2)return'second';if(n===3)return'third';return`${n}th`}
  ...
  function enhance(){
    ...
    if(section){
      const h=section.querySelector('h1'),meta=section.querySelector('.muted');
      setText(h,`Training Day ${n} this week`);
      setText(meta,active?'Workout in progress':'Choose a focus');
      let note=section.nextElementSibling;
      if(!note?.classList?.contains('weekly-flex-note')){note=document.createElement('p');note.className='muted flexible-workout-note weekly-flex-note';section.insertAdjacentElement('afterend',note)}
      if(active){
        setHtml(note,`You already started <strong>${cleanFocus(active.day_snapshot?.name||'a workout')}</strong>. Finish it or use Change Workout to choose another focus.`);
      }else{
        const suggestion=pick?` <strong>Suggested next:</strong> ${cleanFocus(pick.name)}.`:'';
        setHtml(note,`Day ${n} means your ${ordinal(n)} training session this week — not a fixed muscle group. Choose any focus below.${suggestion}`);
      }
    }
    ...
    document.querySelectorAll('.day-card [data-action="start"]').forEach(b=>{const m=b.textContent.match(/^Start\s+(.+)$/);if(m){const next='Start '+cleanFocus(m[1]);if(b.textContent!==next)b.textContent=next}});
