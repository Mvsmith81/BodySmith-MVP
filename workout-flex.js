(()=>{
  ...
  function enhance(){
    ...
    if(section){
      ...
      h.textContent=`Training Day ${n} this week`;
      if(meta)meta.textContent=active?'Workout in progress':'Choose a focus';
      ...
      if(active){
        note.innerHTML=`You already started <strong>${cleanFocus(active.day_snapshot?.name||'a workout')}</strong>. Finish it or use Change Workout to choose another focus.`;
      }else{
        const suggestion=pick?` <strong>Suggested next:</strong> ${cleanFocus(pick.name)}.`:'';
        note.innerHTML=`Day ${n} means your ${ordinal(n)} training session this week — not a fixed muscle group. Choose any focus below.${suggestion}`;
      }
    }
