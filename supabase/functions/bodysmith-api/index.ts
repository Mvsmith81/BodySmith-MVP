...
async function getUser(userId: string) {
  const { data, error } = await db.from("app_users").select("id,username,display_name,units,active_plan_id,created_at,preferences,onboarding_completed,email,auth_user_id").eq("id", userId).single();
  if (error) throw error;
  return data;
}
